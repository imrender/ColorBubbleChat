...
--[[
	// SECTION: Bubble Chat Color
	// Supports team, whisper and 'CombatChat' by ImRender.
	// This section of code is licensed under 'The Unlicense'.
	// A copy of this license is provided at the end of this section. 
	
]]

function calculateBubbleImageColor(sourcePlayer: Player, messageData: any)
	if messageData.OriginalChannel then
		if messageData.OriginalChannel == "Team" then
			return sourcePlayer.TeamColor.Color
		end
		if messageData.OriginalChannel == "Combat" then
			return Color3.new(0.101961, 0.639216, 0.576471)
		end
		if messageData.OriginalChannel:find("To ") or messageData.OriginalChannel:find("From ") then
			return Color3.new(0.870588, 0.129412, 0.796078)
		end
	end
...
--[[
	// END OF SECTION: Bubble Chat Color
	// License:
...
local function distanceToBubbleOrigin(origin)
	...
	return (origin.Position - (workspace.CurrentCamera :: Camera).CFrame.Position).magnitude
end
...
function this:CreateChatLineRender(instance, line, onlyCharacter, fifo, shouldAutoLocalize, imageColor: Color3?, textColor: Color3?)
	...
	if billboardGui then
		...
		if imageColor then
			chatBubbleRender.ImageColor3 = imageColor

			local tail = chatBubbleRender:FindFirstChild("ChatBubbleTail")
			if tail then
				tail.ImageColor3 = imageColor
			end
		end
...
function this:OnPlayerChatMessage(sourcePlayer: Player, messageData: any)
	...
	local safeMessage = this:SanitizeChatLine(messageData.Message)
	...
	if sourcePlayer and line.Origin then
		...
		local imageColor: Color3 = calculateBubbleImageColor(sourcePlayer, messageData)

		local textColor: Color3 = Color3.new(0,0,0)
		if (imageColor.R + imageColor.G + imageColor.B) / 3 < .5 then
			textColor = Color3.new(1,1,1)
		end

		this:CreateChatLineRender(sourcePlayer.Character, line, true, fifo, false, imageColor, textColor)
	end
end
...
OnNewMessage.OnClientEvent:connect(function(messageData, channelName)
	...
	this:OnPlayerChatMessage(sender, messageData)
end)
